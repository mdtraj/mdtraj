...
def box_vectors_to_lengths_and_angles(a, b, c):
    ...
    alpha = np.arccos(np.einsum("...i, ...i", b, c) / (b_length * c_length), dtype=np.float64)
    beta = np.arccos(np.einsum("...i, ...i", c, a) / (c_length * a_length), dtype=np.float64)
    gamma = np.arccos(np.einsum("...i, ...i", a, b) / (a_length * b_length), dtype=np.float64)
