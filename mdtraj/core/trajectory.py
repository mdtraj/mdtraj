...
def load(filename_or_filenames, discard_overlapping_frames=False, **kwargs):
    ...
    kwargs["top"] = _parse_topology(
        kwargs.get("top", filename_or_filenames[0]), **topkwargs
    )
...
class Trajectory(object):
    ...
    def time(self, value):
        ...
        if np.isscalar(value) and self.n_frames == 1:
            ...
            raise ValueError(
                "Wrong shape. Got %s, should be %s" % (value.shape, (self.n_frames))
            )
    ...
    def superpose(
        ...
        ref_align_xyz = np.array(
            reference.xyz[frame, ref_atom_indices, :], copy=True, order="c"
        ).reshape(1, -1, 3)
    ...
    def __init__(
        self, xyz, topology, time=None, unitcell_lengths=None, unitcell_angles=None
    ):
        ...
        with HDF5TrajectoryFile(filename, "w", force_overwrite=force_overwrite) as f:
            f.write(
                coordinates=in_units_of(
                    self.xyz, Trajectory._distance_unit, f.distance_unit
                ),
                time=self.time,
                cell_lengths=in_units_of(
                    self.unitcell_lengths, Trajectory._distance_unit, f.distance_unit
                ),
        ...
        with PDBTrajectoryFile(filename, "w", force_overwrite=force_overwrite) as f:
            for i in xrange(self.n_frames):
                ...
                if self._have_unitcell:
                    f.write(
                        in_units_of(
                            self._xyz[i], Trajectory._distance_unit, f.distance_unit
                        ),
                        self.topology,
                        modelIndex=i,
                        bfactors=bfactors[i],
    ...
    def save_xtc(self, filename, force_overwrite=True):
        ...
        with XTCTrajectoryFile(
            os.fspath(filename), "w", force_overwrite=force_overwrite
        ) as f:
            f.write(
                xyz=in_units_of(self.xyz, Trajectory._distance_unit, f.distance_unit),
                time=self.time,
                box=in_units_of(
                    self.unitcell_vectors, Trajectory._distance_unit, f.distance_unit
                ),
            )
    ...
    def save_mdcrd(self, filename, force_overwrite=True):
        ...
        with MDCRDTrajectoryFile(
            filename, mode="w", force_overwrite=force_overwrite
        ) as f:
            f.write(
                xyz=in_units_of(self.xyz, Trajectory._distance_unit, f.distance_unit),
                cell_lengths=in_units_of(
    ...
    def save_netcdfrst(self, filename, force_overwrite=True):
        ...
        if self.n_frames == 1:
            with AmberNetCDFRestartFile(
                filename, "w", force_overwrite=force_overwrite
            ) as f:
                coordinates = in_units_of(
                    self._xyz,
                    Trajectory._distance_unit,
    ...
    def save_amberrst7(self, filename, force_overwrite=True):
        ...
        if self.n_frames == 1:
            with AmberRestartFile(filename, "w", force_overwrite=force_overwrite) as f:
                coordinates = in_units_of(
                    self._xyz, Trajectory._distance_unit, AmberRestartFile.distance_unit
                )
                lengths = in_units_of(
                    self.unitcell_lengths,
                    Trajectory._distance_unit,
    ...
    def make_molecules_whole(self, inplace=False, sorted_bonds=None):
        ...
        if sorted_bonds is None:
            ...
            sorted_bonds = np.asarray(
                [[b0.index, b1.index] for b0, b1 in sorted_bonds], dtype=np.int32
            )
