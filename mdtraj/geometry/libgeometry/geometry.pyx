...
def compute_distances(float[:, :, ::1] xyz, float[:, :, ::1] box_matrix, Py_ssize_t[:, ::1] pairs):
    cdef Py_ssize_t n_frames = xyz.shape[0]
    cdef Py_ssize_t n_atoms = xyz.shape[1]
...
def compute_angles(float[:, :, ::1] xyz, float[:, :, ::1] box_matrix, Py_ssize_t[:, ::1] triplets):
    cdef Py_ssize_t n_frames = xyz.shape[0]
    cdef Py_ssize_t n_atoms = xyz.shape[1]
    cdef Py_ssize_t n_triplets = triplets.shape[0]
...
def compute_dihedrals(float[:, :, ::1] xyz, float[:, :, ::1] box_matrix, Py_ssize_t[:, ::1] quartets):
    cdef Py_ssize_t n_frames = xyz.shape[0]
    cdef Py_ssize_t n_atoms = xyz.shape[1]
    cdef Py_ssize_t n_quartets = quartets.shape[0]
