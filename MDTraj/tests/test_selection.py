...
def test_bool():
    ...
    eq(sp.mdtraj_expression, "(a.residue.is_protein and a.residue.is_backbone)")
...
def test_values():
    ...
    eq(sp.mdtraj_expression, "a.residue.index == 4")
    ...
    eq(sp.mdtraj_expression, "5 <= a.residue.index <= 8")


def test_not():
    sp = SelectionParser("not protein")
    eq(sp.unambiguous, "(not residue_protein)")
    eq(sp.mdtraj_expression, "(not a.residue.is_protein)")

    sp.parse("not not protein")
    eq(sp.unambiguous, "(not (not residue_protein))")
    eq(sp.mdtraj_expression, "(not (not a.residue.is_protein))")
