...
def baker_hubbard_internal(traj, exclude_water=True, periodic=True):
    """Identify hydrogen bonds based on cutoffs for the Donor-H...Acceptor
    distance and angle.
    ...
    Returns
    -------
    angle_triplets : A list of all D-H-A angle triplets.
    mask : A list of whether a given triplet occurs in each frame.
    ...
    return angle_triplets, mask

def baker_hubbard_list(traj, exclude_water=True, periodic=True):

    """ Identify hydrogen bonds in the same way as baker_hubbard, but
    return a list of hydrogen bonds for each frame. 
    ...
    angle_triplets, mask = baker_hubbard_internal(traj, exclude_water, periodic)
