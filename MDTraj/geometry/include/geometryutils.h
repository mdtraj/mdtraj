...
static INLINE void loadBoxMatrix(const float M[9], __m128 h[3], __m128 hinv[3])
{
    ...
    /* format. */
    inverse33(M, &hinv[0], &hinv[1], &hinv[2]);
}
 ...
 * vector (__m128) `r`. `h` and `hinv` are arrays of three float4s which
 * specify the matricies H and H^{-1} in column-major order. That is,
 * the first float4 h[0] gives [H[0,0], H[1,0], H[2,0]. 0], h[2]
 * is [H[0,1], H[1,1], H[2,1], 0], etc.
 *
