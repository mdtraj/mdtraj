...
def lengths_and_angles_to_box_vectors(a_length, b_length, c_length, alpha, beta, gamma):
    ...
    Examples
    --------
    >>> lengths_and_angles_to_box_vectors(1, 1, 1, np.pi/2, np.pi/2, np.pi/2)
    ...
    if np.all(alpha > 2*np.pi) and np.all(beta > 2*np.pi) and np.all(gamma > 2*np.pi):
        warnings.warn('All your angles were greater than 2*pi. Did you accidently give me degrees?')
...
def box_vectors_to_lengths_and_angles(a, b, c):
    ...
    True
    >>> np.abs(alpha - np.pi / 4) < 1e-10
    True
    >>> np.abs(beta - np.pi / 2) < 1e-10 and np.abs(gamma - np.pi / 2) < 1e-10
    True
