...
@pytest.mark.skipif(
    np.__version__ < "2.0", reason="Expected failure for NumPy < 2.0 due to histogram output differences"
)
...
def test_compare_n_concurrent_pairs(get_fn):
    ...
    r_t_1, rdf_O_O_1 = mdtraj.geometry.rdf.compute_rdf_t(
        traj,
        pairs,
        times,
        n_concurrent_pairs=100000,
    )
    r_t_2, rdf_O_O_2 = mdtraj.geometry.rdf.compute_rdf_t(
        traj,
        pairs,
        times,
        n_concurrent_pairs=314159,
    )
